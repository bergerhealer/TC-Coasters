...
/**
 * Shows a grid of blocks that can be selected using W/A/S/D
 */
public class MapWidgetBlockGrid extends MapWidget implements ItemDropTarget {
    private int _columns = 4;
    private int _rows = 3;
    private int _itemSize = 16;
    private MapBlockTextureCache _textures = MapBlockTextureCache.get(16, 16);
    private int _itemSpacing = 1;
    private int _scrollOffset = 0;
    private int _selectedIndex = 0;
    private List<Material> _blocks = new ArrayList<Material>();
    ...
    /**
     * Gets the currently selected block material
     * 
     * @return selected block material
     */
    public Material getSelectedBlock() {
        return (_selectedIndex >= 0 && _selectedIndex < _blocks.size()) ?
                _blocks.get(_selectedIndex) : null;
    }

    /**
     * Sets the currently selected block material. If the block does not
     * exist in the added items, no item is selected.
     * 
     * @param block to select
     */
    public void setSelectedBlock(Material block) {
        // Find the item
    ...
    public MapWidgetBlockGrid setItemSize(int itemSize) {
        ...
        this._textures = MapBlockTextureCache.get(itemSize, itemSize);
        this.calcSize();
        this.invalidate();
        return this;
    }
    ...
    /**
     * Adds all the block materials available
     * 
     * @return this block grid widget
     */
    ...
    /**
     * Adds a new block material to be displayed in the grid
     * 
     * @param block
     * @return this item grid widget
     */
        ...
        for (int row = 0; row < this._rows; row++) {
            for (int col = 0; col < this._columns; col++) {
                ...
                if (index >= 0 && index < this._blocks.size()) {
                    view.draw(_textures.get(BlockData.fromMaterial(this._blocks.get(index))), x, y);
